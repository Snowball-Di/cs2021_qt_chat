...
QSize chatmessagebox::getRealString(QString src)
{
...
void chatmessagebox::paintEvent(QPaintEvent *event)
{
    ...
    if(m_userType == Text_Type::User_She) { // 用户
        //头像
//        painter.drawRoundedRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
    ...
    }  else if(m_userType == Text_Type::User_Me) { // 自己
        //头像
//        painter.drawRoundedRect(m_iconRightRect,m_iconRightRect.width(),m_iconRightRect.height());
    ...
    }  else if(m_userType == Text_Type::User_Time) { // 时间
        QPen penText;
